#!/usr/bin/env bash
# Update genesis.json with production parameters BEFORE gentx creation

GENESIS_FILE="$HOME/.pos/config/genesis.json"
echo "Updating genesis with production parameters: $GENESIS_FILE"

# Create backup
cp "$GENESIS_FILE" "$GENESIS_FILE.backup"

# STAKING PARAMETERS (lines around "staking")
# max_validators: 100 -> 125
sed -i 's/"max_validators": 100,/"max_validators": 125,/g' "$GENESIS_FILE"

# min_commission_rate: "0.000000000000000000" -> "0.050000000000000000"
sed -i 's/"min_commission_rate": "0.000000000000000000"/"min_commission_rate": "0.050000000000000000"/g' "$GENESIS_FILE"

# SLASHING PARAMETERS
# signed_blocks_window: "100" -> "30000"
sed -i 's/"signed_blocks_window": "100"/"signed_blocks_window": "30000"/g' "$GENESIS_FILE"

# min_signed_per_window: "0.500000000000000000" -> "0.050000000000000000"
sed -i 's/"min_signed_per_window": "0.500000000000000000"/"min_signed_per_window": "0.050000000000000000"/g' "$GENESIS_FILE"

# slash_fraction_downtime: "0.010000000000000000" -> "0.000100000000000000"
sed -i 's/"slash_fraction_downtime": "0.010000000000000000"/"slash_fraction_downtime": "0.000100000000000000"/g' "$GENESIS_FILE"

# MINT PARAMETERS
# blocks_per_year: "6311520" -> "5256000"
sed -i 's/"blocks_per_year": "6311520"/"blocks_per_year": "5256000"/g' "$GENESIS_FILE"

# GOVERNANCE PARAMETERS
# voting_period: "172800s" -> "432000s" (5 days)
sed -i 's/"voting_period": "172800s"/"voting_period": "432000s"/g' "$GENESIS_FILE"

echo ""
echo "✓ Genesis parameters updated successfully!"
echo ""
echo "Verification:"
echo "============="
grep -A 7 '"staking"' "$GENESIS_FILE" | grep -E '(max_validators|min_commission_rate)' | head -2
grep -A 7 '"slashing"' "$GENESIS_FILE" | grep -E '(signed_blocks_window|min_signed_per_window|slash_fraction_downtime)' | head -3
grep -A 7 '"mint"' "$GENESIS_FILE" | grep 'blocks_per_year'
grep -A 30 '"gov"' "$GENESIS_FILE" | grep 'voting_period' | head -1
